...
function LinkToUserGroup(props: LinkToUserGroupProps) {
  ...
  const subjectStructure = subject.subjectStructure

  const { data: usergroups } = useSWR(
    subjectStructure && org ? `${getAPIUrl()}usergroups/org/${org.id}` : null,
      ...
      <div className="p-4 flex-row flex justify-between items-center">
        ...
        <div className="py-3">
          <button
            ...
            className="bg-green-700 text-white font-bold px-4 py-2 rounded-md shadow"
          >
            Link
          </button>
        </div>
