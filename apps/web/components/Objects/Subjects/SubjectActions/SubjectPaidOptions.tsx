...
interface SubjectPaidOptionsProps {
  subject: {
    id: string
    org_id: number
  }
...
function SubjectPaidOptions({ subject }: SubjectPaidOptionsProps) {
  ...
  const { data: linkedProducts, error } = useSWR(
    () =>
      org && session
        ? [
            `/payments/${subject.org_id}/subjects/${subject.id}/products`,
            session.data?.tokens?.access_token,
          ]
        : null
    // ([url, token]) => getProductsBySubject(subject.org_id, subject.id, token)
  )
  ...
  const handleCheckout = async (productId: number) => {
    ...
    try {
      ...
      const redirect_uri = getUriWithOrg(org.slug, '/subjects')
...
export default SubjectPaidOptions
