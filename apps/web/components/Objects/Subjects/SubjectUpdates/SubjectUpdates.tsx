...
import {
  createSubjectUpdate,
  deleteSubjectUpdate,
} from '@services/subjects/updates'
...
function SubjectUpdates() {
  ...
  const { data: updates } = useSWR(
    `${getAPIUrl()}subjects/${subject?.subjectStructure.subject_uuid}/updates`,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const body = {
        ...
        subject_uuid: subject.subjectStructure.subject_uuid,
        org_id: org.id,
      }
      const res = await createSubjectUpdate(
        body,
...
const DeleteUpdateButton = ({ update }: any) => {
  ...
  const handleDelete = async () => {
    const res = await deleteSubjectUpdate(
      subject.subjectStructure.subject_uuid,
      update.subjectupdate_uuid,
      session.data?.tokens?.access_token
    )
